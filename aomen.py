...
html_file_name1 = timestr+"/am1"+timestr+".html" #每次上传使用时间戳目录 timestr，方便后续一次性删除
html_file_name2 = timestr+"/am2"+timestr+".html" #每次上传使用时间戳目录 timestr，方便后续一次性删除
html_file_name3 = timestr+"/am3"+timestr+".html" 
html_file_name4 = timestr+"/am4"+timestr+".html"
html_file_name5 = timestr+"/am5"+timestr+".html"
html_file_name_arr = [html_file_name1, html_file_name2,html_file_name3, html_file_name4, html_file_name5]

data_json_name1 = timestr+"/am1"+timestr+".json"
data_json_name2 = timestr+"/am2"+timestr+".json"
data_json_name3 = timestr+"/am3"+timestr+".json"
data_json_name4 = timestr+"/am4"+timestr+".json"
data_json_name5 = timestr+"/am5"+timestr+".json"

html_file_name_jianming = "lb简明版.html"
html_file_name_old1 = ""
data_json_name_old1 = ""
html_file_name_old2 = ""
data_json_name_old2 = ""
...
IMAGE_URL = "https://tk2.sycccf.com:4949/col/"+qishu

IMAGE_URL1 = "https://tk2.sycccf.com:4949/col/"+qishu+"/rv.jpg"

file_object = open("需要发给老爸的图.txt", 'r')  # 创建一个文件对象，也是一个可迭代对象
try:
...
def urllib_download():
    with open('./data.json', 'w', encoding='utf-8') as f:
        ...
        for index, name in enumerate(imageNameArr_aomen):
            ...
            print("==================正在下载第【"+str(i)+"】图片=================")
            urlretrieve(url, './image'+name)
            print("==================图片地址是【"+url+"】=================")
        arr.append("image/rv.jpg")
        urlretrieve(IMAGE_URL1, './image/rv.jpg')
    load_dict["ImgList"] = arr
...
def request_download():
    ...
    for index, name in enumerate(imageNameArr_aomen):
        ...
        with open('./image'+name, 'wb') as f1:
            f1.write(r.content)
    try:
        r = requests.get(IMAGE_URL1, headers=headers, timeout=3)
    except Exception:
        print("==================正在下载图片【"+name+"】，下载失败=================")
        # load_dict["ImgList"] = arr
        # json.dump(load_dict, f, indent=4, ensure_ascii=False)
        # return

    with open('./image/rv.jpg', 'wb') as f1:
        f1.write(r.content)
...
description = '''
    ...
    💪今晚必中！
    '''
shengyushuliang = len(uploaded_image_arr)-240
aomenImgUrl = "https://s1.ax1x.com/2023/01/03/pSiMmlV.png"
hkImgUrl = "https://s1.ax1x.com/2023/01/03/pSiMYSx.png"
...
for i in range(1, 6):
    ...
    send_message(accessToken, title1, description1,
                 html_file_name_arr[i-1], aomenImgUrl)
    time.sleep(2)

time.sleep(2)
#程序结束运行时间
end_time_str = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
send_message(accessToken, "澳门资料第"+qishu + "期", "开始时间：" +
             start_time_str+"，结束时间："+end_time_str, "", aomenImgUrl)
os.system("pause")
